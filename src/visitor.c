...
static AST_T* builtin_function_print(visitor_T* visitor, AST_T** args, int args_size, char* newline)
{
  for(int i = 0; i < args_size; ++i)
  {
    ...
    switch (visited_ast->type)
    {
      ...
      case AST_ARRAY: builtin_function_print(visitor, visited_ast->array, visited_ast->array_size, " "); printf("\n"); break;
      default: printf("%p%s", visited_ast, newline); break;
    }
...
AST_T* visitor_visit_variable_definition(visitor_T* visitor, AST_T* node)
{
  ...
  return node;
  // if(visitor->env_list_head->variable_definitions == (void*) 0)
  // {
...
AST_T* visitor_visit_assignment(visitor_T* visitor, AST_T* node)
{
  ...
  while(curr_env!= (void*) 0)
  {
    for(int i=0; i<curr_env->variable_definitions_size; ++i)
    {
      ...
      {
        vardef->variable_definition_value = visitor_visit(visitor, node->new_assignment_value);
        return visitor_visit(visitor, vardef->variable_definition_value);
      }
    }
...
AST_T* visitor_visit_array_access(visitor_T* visitor, AST_T* node)
{
  ...
  {
    printf("Variable %s is not an array\n", node->array_access_variable_name);
    exit(1);
  }

  if(index->literal_value >= array_ast->array_size)
  {
    printf("index %d is out of bounds for array %s\n", index->literal_value, variable_ast->variable_name);
    exit(1);
  }
...
AST_T* visitor_visit_array_assignment(visitor_T* visitor, AST_T* node)
{
  ...
  if(array_ast->type != AST_ARRAY)
  {
    printf("Variable %s is not an array\n", node->array_assignment_variable_name);
    exit(1);
  }
...
// AST_T* visitor_visit_else_if_statement(visitor_T* visitor, AST_T* node)
// {
//   if(node->if_statement_predicate->boolean_value == true)
//   {
//     visitor_visit_lbrace(visitor, node);
//     visitor_visit(visitor, node->if_statement_body);
//     visitor_visit_rbrace(visitor, node);
